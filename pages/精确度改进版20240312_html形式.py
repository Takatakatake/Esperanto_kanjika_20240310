...
def conversion_format(hanzi,word):
    ...
    for line in file:
        ...
        if len(j)==4:
            ...
            if ("#" in word) or word=='':
                ...
                replacements_dict[word]=[conversion_format(hanzi,word),len(word)*10000+5000]##为了优先于不直接转换的世界语词根进行汉字替换，将替换顺序加+5000~(+4600,+4800,+5000)。
                ##実際に漢字置換するエスペラント語根をそのまま変換しないエスペラント語根より優先するため、置換順序を+5000~(+4600,+4800,+5000)する。
            elif any(keyword in level for keyword in ['尾','数','前']):
                replacements_dict[word]=[conversion_format(hanzi,word),len(word)*10000+4800]##为了下述的"提高汉字替换准确性的工夫"，根据世界语词根的词性，稍微调整替换顺序。(後述の"漢字置換の精度を高める工夫"のために、エスペラント語根の品詞によって、置換順序を多少前後させる。)
                ...
                replacements_dict[word]=[conversion_format(hanzi,word),len(word)*10000+4800]
            else:
                replacements_dict[word]=[conversion_format(hanzi,word),len(word)*10000+4600]##其他进行汉字替换的世界语词根不应用"提高汉字替换准确性的工夫"，因此加4600。(その他の漢字置換するエスペラント語根は、"漢字置換の精度を高める工夫"を適用しないので、+4600する。)
        ...
        if len(j)==4:
            ...
            if level in HANZILEVEL:
                if word==hanzi:
                    ...
                    replacements_dict[word]=[conversion_format(hanzi,word),int(priority)]
